...
public class BossDash : MonoBehaviour
{
    ...
    private float itemForce = 10f;
    private float itemUpwardForce = 10f;
    private float cantidadItems = 10f;
    ...
    private IEnumerator UsarHabilidadEspecial()
    {
        ...
        float dashDirection = mirandoDerecha ? 1f : -1f;
        rb.velocity = new Vector2(dashDirection * dashForce, rb.velocity.y);

        // Expulsar ítems durante el dash
        for (int i = 0; i < 5; i++)
        {
            if (proyectilPrefab != null)
            {
                GameObject item = Instantiate(proyectilPrefab, transform.position, Quaternion.identity);
                Rigidbody2D itemRb = item.GetComponent<Rigidbody2D>();
                if (itemRb != null)
                {
                    // 🚀 Utilizamos las fuerzas AUMENTADAS: itemForce (horizontal) e itemUpwardForce (vertical).
                    Vector2 throwDirection = new Vector2(
                        dashDirection * itemForce + Random.Range(-1f, 1f), // Dirección del dash * nueva fuerza horizontal
                        itemUpwardForce + Random.Range(0f, 2f) // Nueva fuerza vertical
                    );
                    itemRb.AddForce(throwDirection, ForceMode2D.Impulse);
                }
            }
            yield return new WaitForSeconds(dashDuration / cantidadItems);
        }

        // Mantenemos la inercia del dash hasta el final de su duración
        yield return new WaitForSeconds(dashDuration - (cantidadItems * (dashDuration / cantidadItems)));

        atacando = false; // Permite que el MoverYSeguirJugador tome el control nuevamente
